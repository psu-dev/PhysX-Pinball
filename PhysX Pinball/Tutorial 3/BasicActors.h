...
namespace PhysicsEngine
{
	...
	// Static Box actor for plunger bottom
	...
	// Static Capsule actor for bumpers
	class CapsuleStatic : public StaticActor
	{
	public:
		CapsuleStatic(const PxTransform& pose = PxTransform(PxIdentity), PxVec2 dimensions = PxVec2(1.f, 1.f), PxReal density = 1.f)
			: StaticActor(pose)
		{
			CreateShape(PxCapsuleGeometry(dimensions.x, dimensions.y), density);
		}
