...
namespace VisualDebugger
{
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		camera = new Camera(PxVec3(0.0f, 12.0f, 15.f), PxVec3(0.f,-1.f,-2.f), 5.f);
	...
	void HUDInit()
	{
		...
		// Add instructions
		hud.AddLine(HELP, "Use 'N' and 'M' for left and right flippers respectively.");
		hud.AddLine(HELP, "Press 'Space' to use the plunger.");
		hud.AddLine(HELP, "Aim for the longest time possible!");
		hud.AddLine(EMPTY, "");

		// Add time and previous time
		hud.AddLine(HELP, "Time: ");
		hud.AddLine(HELP, "Best time: ");
	...
	void RenderScene()
	{
		...
		// Update the timer and best time
		hud.UpdateLine(0, "Time: " + to_string(scene->timer), 3);
		hud.UpdateLine(0, "Best time: " + to_string(scene->prevTime), 4);
	...
	void UserKeyPress(int key)
	{
		switch (toupper(key))
		{
		// Inputs for flippers and plunger force
		case 'N':
			scene->FlipperForce(0, 10.f);
			break;
		case 'M':
			scene->FlipperForce(1, -10.f);
			break;
		case ' ':
			scene->PlungerForce(40.f);
			break;
		default:
			break;
		}
	...
	void UserKeyRelease(int key)
	{
		switch (toupper(key))
		{
		// Reset flippers with negative force
		case 'N':
			scene->FlipperForce(0, -10.f);
		case 'M':
			scene->FlipperForce(1, 10.f);
